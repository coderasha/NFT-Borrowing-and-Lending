...
//           const callData = this.tribeOne
//             .connect(this.signers[1])
//             .interface.encodeFunctionData('postLiquidation', [this.loanId, soldAmount]);

//           // console.log('multisigwallet', this.multiSigWallet.address, this.signers[1].address);
//           // await this.multiSigWallet.submitTransaction(this.tribeOne.address, soldAmount, callData, { value: soldAmount });
//           // await this.multiSigWallet.confirmTransaction(2, false);
//           // await this.multiSigWallet.connect(this.signers[1]).confirmTransaction(2, false);

//           // await this.multiSigWallet.executeTransaction(2);
...
//       await expect(this.multiSigWallet.executeTransaction(0))
//         .to.emit(this.tribeOne, 'LoanApproved')
//         .withArgs(loanId, this.agent.address, this.loanCurrency.address, amount);
//     });
...
//         await expect(this.multiSigWallet.executeTransaction(txId))
//           .to.emit(this.tribeOne, 'LoanApproved')
//           .withArgs(this.loanId, this.agent.address, this.loanCurrency.address, amount);
...
//         await this.loanCurrency.connect(this.agent).increaseAllowance(this.tribeOne.address, loanAmount);
...
//       /**
//        * Current Loan - [6, 2500, 300] 10000- 100% Tenor LTV, interest, nftTypes - [ERC721, ERC721, ERC1155], nftIds - [1, 2, 10]
//        * collateral: 100 usdc, fund amount: 1 usdc, loan amount: 1.2 usdc
//        */
//       describe('Loan payment', function () {
//         beforeEach(async function () {
//           const nftTokenTypeArray = [NFT_TYPE.ERC721, NFT_TYPE.ERC721, NFT_TYPE.ERC1155];
...
//         it('Pay installment and withdraw NFT without any penalty', async function () {
//           // Paid 0.2 usdc for installment
//           const totalDebt = await this.tribeOne.totalDebt(this.loanId);
//           const desiredAmount = totalDebt.div(6);

//           for (let ii = 0; ii < 5; ii++) {
//             await this.loanCurrency.connect(this.alice).increaseAllowance(this.tribeOne.address, desiredAmount);
//             await expect(this.tribeOne.connect(this.alice).payInstallment(this.loanId, desiredAmount))
//               .to.emit(this.tribeOne, 'InstallmentPaid')
//               .withArgs(this.loanId, this.alice.address, this.loanCurrency.address, desiredAmount);
//           }
...
//         it('Put NFT in Liquidation and user get back the rest', async function () {
//           // Pay 0.2 usdc for one installment
//           const totalDebt = await this.tribeOne.totalDebt(this.loanId);
//           const desiredAmount = totalDebt.div(6);
//           let createdLoan = await this.tribeOne.loans(this.loanId);
//           const loanStart = createdLoan.loanStart;
...
//           await this.loanCurrency.connect(this.salesManager).increaseAllowance(this.tribeOne.address, soldAmount);
//           await expect(this.tribeOne.connect(this.salesManager).postLiquidation(this.loanId, soldAmount))
//             .to.emit(this.tribeOne, 'LoanPostLiquidation')
//             .withArgs(this.loanId, soldAmount, finalDebt);

//           // user will get back 2 usdc - finalDebt;
//           // @note all fee constants are zero
//           await expect(this.tribeOne.connect(this.alice).getBackFund(this.loanId))
//             .to.emit(this.tribeOne, 'RestWithdrew')
//             .withArgs(this.loanId, soldAmount.sub(finalDebt));
